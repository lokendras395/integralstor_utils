...
def is_mounted(disk_name):
    ...
    try:
        ...
        output, err = command.get_command_output(
            cmd, shell=True, check_rc=False)
        if err:
            raise Exception(err)
        elif output == []:
            is_mounted = False
...
def get_file_system_by_name(disk_name):
    """get file system type by providing disk name or partition name"""
    fs_type = None
    try:
        ...
        if len(fstype_list) == 1:
            ...
            raise Exception(
                "It looks like you have multiple partitions on %s. \nPlease enter the partition name (eg. /dev/sdx1)" % disk_name)
    except Exception, e:
...
def mount_disk_by_name(disk_name, path=None):
    """Mount the disk by disk name, returns True if successfullly mounted, else return error"""
    try:
        ...
        return False, "Unable to mount disk %s : %s" % (disk_name, str(e))
    else:
        return True, None
...
def get_mount_point_by_name(disk_name):
    ...
    try:
        ...
        if output is not None:
            if len(output) > 1:
                raise Exception("Disk have multiple partition, enter partiton name")
            else:
...
def unmount_disk_by_name(disk_name):
    """Unmount disk by name"""
    try:
        outputt, err = is_mounted(disk_name)
        if err:
            raise Exception(err)
        elif outputt == True:
